...
var currentRotationPosition = [0, -25, 0];
var rotation = 0;
...
export default class Globe extends Component {
  constructor(props) {
    ...
    this.currentProjection =
      this.props.Projection.function || d3.geoOrthographic();
    ...
    if (!this.pauseAutoRotation) {
      window.requestAnimationFrame(() => {
        ...
        this.projection.rotate([
          currentRotationPosition[0] + rotation,
          currentRotationPosition[1],
          currentRotationPosition[2],
        ]);
        this.renderWorld(world, context, path, canvas);
      });

      if (this.recreateGlobe) {
        this.recreateGlobe = false;
        return;
      }
    }
  ...
  renderNewPointOnGlobe = (point) => {
    ...
    let anim = circle.animate([{ r: "0px" }, { r: "5px" }, { r: "0px" }], {
  ...
  createPoint() {
    ...
    this.currentPoint++;
    return [
      availablePoints[this.currentPoint % availablePoints.length].coords
        .longitude,
      availablePoints[this.currentPoint % availablePoints.length].coords
        .latitude,
    ];
  ...
  PointSpawning = () => {
    ...
    if (!this.exit)
      setTimeout(this.PointSpawning, this.randomIntFromInterval(10, 90));
  };

  PollMarkersUnAuthenticaated = async () => {
    let endpoint = new URL(
      "https://testifyheatmap.azurewebsites.net/api/GetLatestCoordinatesUnAuthenticated?&timeSpan=10080"
    );
  ...
  zoom(
    ...
    function zoomed(event) {
      ...
      currentRotationPosition = versor.rotation(q1);
      rotation = 0;
